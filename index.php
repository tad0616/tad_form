...
function sign_form($ofsn="",$ssn=""){
  ...
  $ofsn=$form['ofsn'];
    ...
    $db_ans_csn=isset($db_ans[$csn])?$db_ans[$csn]:"";
	  $col_form=col_form($csn,$kind,$size,$val,$db_ans_csn);
...
//製作表單
function col_form($csn="",$kind="",$size="",$default_val="",$db_ans=array()){


	switch($kind){
		case "text":
		$default_val=(empty($db_ans))?$default_val:$db_ans;
		$span=round($size/10,0);
		$main="<input type='text' name='ans[$csn]' id='tf{$csn}' span='span{$span}' value='{$default_val}'><input type='hidden' name='need_csn[{$csn}]' value='{$csn}'>";
		break;
      ...
      <input type='radio' name='ans[$csn]' value='{$val}' $checked>{$val}
      </label>";
      $i++;
		}
		break;
      ...
      <input type='checkbox' name='ans[$csn][]' value='{$val}' $checked>{$val}
      </label>";
      $i++;
		}
		break;
      ...
      $main.="<option value='{$val}' $selected>{$val}</option>";
		...
		$main="<textarea name='ans[$csn]' id='tf{$csn}' class='span12' style='height:{$size}px;'>{$default_val}</textarea><input type='hidden' name='need_csn[{$csn}]' value='{$csn}'>";
		break;
